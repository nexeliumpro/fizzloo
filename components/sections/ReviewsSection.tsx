    ...
    <section className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-16">
          ...
          <h2 className="font-bold text-4xl text-slate-800 mb-2">{t.sections.reviews}</h2>
          ...
          {REVIEWS.map((review,i) => (
            <div key={i} className="bg-white rounded-3xl p-6 shadow-card border border-slate-100 hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1">
              ...
              <div className="flex items-center justify-between">
                ...
                {review.verified && <span className="text-xs font-bold text-brand-500 bg-blue-50 px-2 py-1 rounded-full">✓ {locale==='fr'?'Vérifié':'Verified'}</span>}
              </div>
            </div>
          ))}
        </div>
